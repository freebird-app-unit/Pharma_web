...
function getreportorderlist(pageno){
	...
	$.ajax({
		...
		success: function (resp) {
			$('#total_amount').text(0);
			$('#pending_amount').text(0);
			
			if(user_type == 'admin'){
				$('#pending_collection').text(0);
			}
			...
			if(user_type == 'admin'){
				$('#pending_collection').text(resp.pending_collection);
			}
        }
	})
	...
	if(order_type == 1){
		...
		if(user_type == 'admin' && typeof pharmacy_id !== 'undefined' && pharmacy_id !== ''){
		...
		if(user_type == 'admin' && typeof logistic_id !== 'undefined' && logistic_id !== ''){
			$('#pay').prop('disabled', false);
		} else if (user_type == 'admin') {
			$('#pay').prop('disabled', true);
		}
	...
	$.ajax({
		type: "post",
		url: base_url+'/getreportorderlist',
