	...
	$.fn.formBuilder = function(options) {
		...
		*/
		var fieldSelect = function() {
			...
			$(document).on('click', '.form-element', function(event) {
				//$('.form-element').click(function(){
				event.stopImmediatePropagation();
				// Remove selected class from all elements
				clearSelectedElements();
				
				//alert($(this).addClass('sdsadasdsadassdsadasdasdasasda'));
				// Add selected class to selected element
				$(this).addClass('selected');
				$('html, body').animate({scrollTop: $(".selected").offset().top - 200}, 20);
			...
			$('.bind-control').each(function(){
				...
				$(this).on("keyup", function() {
					if(currentlySelected == '') {
						...
						if( target == '#form-title-label' ) {
							$(target).html($(this).val());
						}else if( target == '#form-description-label' ) {
							$(target).html($(this).val());
						}else if( currentlySelected.data('type') != 'element-dropdown' ) {
		...
		*/
		var bindSettings = function() {	
			...
			// Description
			if( currentlySelected.data('type') == 'element-section-break' || currentlySelected.data('type') == 'element-single-line-text') {
		...
		var rules = function() {
			...
			$("#control-add-rule").click(function(){
				...
				$('.form-element').each(function(){
					...
					var label = $(this).children('label').children('.label-descrioption').html();
		...
		var controlMultipleChoice = function() {
			...
			$('.add-choice').click(function(){
				...
				dust.render(currentlySelected.children('.choices').data('type'), data, function(err, out) {
					...
					var elementId = currentlySelected.attr('data-position').replace('element-','');
					// Load template
					data = {
						'title': 'Untitled',
						'value': 'untitled',
		...
		*/
		...
		dust.onLoad = function(name, callback) {
		  $.ajax(base_url+'/public/admin/formbuilder/templates/' + name + '.tpl', {
		...
		$.getJSON( settings.load_url, function( data ) {
			//data = '{"title":"Create Form 1111111111111111111","description":"Demo 1111111111111111111111111","fields":[],"rules":[]}';
				...
				$('.loading').fadeOut(function(){
					...
					$('#save').click(function(e){
						...
						if (form_title.trim() == '') {
							tabs.showTab('#form-settings');
							$('#form-settings-element').addClass('selected');
							$('#form-title_grp').addClass('has-error');
							$('#form-title_err_msg').html(langauge_var.form_title_blank_msg);
							$('#form-title').focus();
							return false;
						}
						$.ajax({
							type: "POST",
							//url: base_url + "add_intakeform",
							url: settings.save_url,
							dataType:'JSON',
							data: {formData: form_data,_token: _token},
							success: function (all_response) { 
								if(all_response.status=='error'){
									$('#main_err_msg').html(all_response.msg);
									return false;
								}else{
									$.toast({
										  heading: langauge_var.common_success_header,
										  showHideTransition: 'slide', 
										  text: all_response.msg,
										  icon: 'success',
										});
									location.href = base_url + "intakeform/list";
									return false;
								}
								/*
								$.toast({
