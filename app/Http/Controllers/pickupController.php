...
class pickupController extends Controller
{
    ...
    public function index()
    {
		...
		$data['page_condition'] = 'page_pickup';
	...
	public function logistic_getlist()
    {
		...
		$order_detail = $order_detail->orderby('new_orders.pickup_datetime','desc');
	...
	public function getlist()
    {
		...
		$order_detail = new_orders::select('new_orders.id','deliveryboy_id','order_number','order_note','new_users.name as customer_name','new_users.mobile_number as customer_number','new_users.id as customerid', 'prescription.name as prescription_name', 'prescription.image as prescription_image','new_orders.pickup_datetime','new_orders.is_external_delivery as is_external_delivery','new_orders.logistic_user_id as logistic_user_id','delivery_user.name as assign_to')
	...
	public function logistic_order_details($id)
    {
		...
		$order_detail = DB::table('new_orders')->select('new_orders.*','new_delivery_charges.delivery_type as delivery_type','new_delivery_charges.delivery_price as delivery_price', 'address_new.address as address','new_pharmacies.address as pharmacyaddress','new_users.name as deliveryboyname')
		->leftJoin('new_users', 'new_users.id', '=', 'new_orders.deliveryboy_id')
		->leftJoin('new_pharmacies', 'new_pharmacies.id', '=', 'new_orders.pharmacy_id')
		->leftJoin('new_delivery_charges', 'new_delivery_charges.id', '=', 'new_orders.delivery_charges_id')
		->leftJoin('address_new', 'address_new.id', '=', 'new_orders.address_id')
		->where('new_orders.order_status','pickup')
