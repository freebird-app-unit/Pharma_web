...
class DeliveryincompleteController extends Controller
{
    ...
    public function index()
    {
		...
		$data['page_condition'] = 'page_deliveryincomplete';
		$data['site_title'] = 'Incomplete order | ' . $this->data['site_title'];
        return view('deliveryincomplete.index', $data);
    }
	public function getlist()
    {
		...
		if(count($order_detail)>0){
			foreach($order_detail as $order){
				...
				$html.='<tr>
					<td><img src="'.$image_url.'" width="50"/><span>'.$order->order_number.'</span></td>
