...
class AcceptorderController extends Controller
{
    ...
    public function accept_order_list(Request $request)
    {
        ...
        if (!empty($user_id) && !empty($search_text)) {
             $name = new_users::where('name', 'like', '%' .$search_text . '%')->get();
             if(count($name)>0){
              foreach ($name as $n) {
    ...
    public function outof_order_list(Request $request){
        ...
        if (!empty($user_id) && !empty($search_text)) {
            ...
            $outof_name = new_orders::where(['process_user_id' => $user_id,'customer_id'=> $n->id])->where(function($query) {
                        $query->where('order_status','assign')
                            ->orWhere('order_status','pickup')
                            ->orWhere('order_status','accept');
                    })->orderBy('assign_datetime', 'DESC')->get();
               }
           }else{
                $outofdelivery_list = new_orders::where('order_number', 'like', '%' .$search_text . '%')->where('process_user_id',$user_id)->where(function($query) {
                        $query->where('order_status','assign')
                            ->orWhere('order_status','pickup')
                            ->orWhere('order_status','accept');
                    })->orderBy('assign_datetime', 'DESC')->get();
            ...
            $outof_deliveryboy = new_orders::where('deliveryboy_id', 'like', '%' .$search_text . '%')->where(['process_user_id' => $user_id,'deliveryboy_id' => $delivery_boy])->where(function($query) {
                        $query->where('order_status','assign')
                            ->orWhere('order_status','pickup')
                            ->orWhere('order_status','accept');
                    })->orderBy('assign_datetime', 'DESC')->get();
        }elseif(!empty($user_id) && !empty($order_status)){
            $order_status_data = new_orders::where('order_status', 'like', '%' .$order_status . '%')->where('process_user_id',$user_id)->orderBy('assign_datetime', 'DESC')->get();
        }else{
               $outofdelivery_list =  new_orders::where('process_user_id',$user_id)->where(function($query) {
                        $query->where('order_status','assign')
                            ->orWhere('order_status','pickup');
                    })->orderBy('assign_datetime', 'DESC')->get();
        }
        ...
        $user = new_pharma_logistic_employee::where(['id'=>$user_id,'api_token'=>$token])->get();
        if(count($user)>0){
                if(!empty($outofdelivery_list)){
                         foreach($outofdelivery_list as $value) {
                                    $prescription_image = '';
                                ...
                                $delivery_name_data = new_pharma_logistic_employee::where('id', $value->deliveryboy_id)->get();
                                $delivery_name = '';
                                foreach ($delivery_name_data as $dn) {
                                        $delivery_name =$dn->name;
                                    }
                                    $outof[] = [
                                    'order_id' => $value->id,
                                    'order_number' => $value->order_number,
                ...
                } else if(!empty($outof_name)){
                        foreach($outof_name as $value) {
                                    $prescription_image = '';
                                    $image_list = Prescription::where('id',$value->prescription_id)->get();
                                    foreach ($image_list as $p_img) {
                                    if (!empty($p_img->image)) {
                        ...
                        $response['status'] = 200;
                        $response['message'] = 'Out Of Delivery List';
                } else if(!empty($order_status_data)){
                        ...
                        $response['status'] = 200;
                        $response['message'] = 'Out Of Delivery List';
                }else if(!empty($outof_deliveryboy)){
                        foreach($outof_deliveryboy as $value) {
                                    $prescription_image = '';
                                    $image_list = Prescription::where('id',$value->prescription_id)->get();
                                    foreach ($image_list as $p_img) {
                                    if (!empty($p_img->image)) {
   ...
   public function reject_order_list(Request $request){
        ...
        if (!empty($user_id) && !empty($search_text)) {
             ...
             if(count($name)>0){
              foreach ($name as $n) {
                   ...
                   $reject_list_name = new_orders::where(['process_user_id' => $user_id, 'order_status' => 'reject','customer_id'=> $n->id])->orderBy('reject_datetime', 'DESC')->get();
               }
           }else{
                $reject_list = new_orders::where('order_number', 'like', '%' .$search_text . '%')->where(['process_user_id' => $user_id, 'order_status' => 'reject'])->orderBy('reject_datetime', 'DESC')->get();
           }
        }else{
               $reject_list =  new_orders::where('process_user_id', $user_id)->where('order_status','reject')->orderBy('reject_datetime', 'DESC')->get();
        }
        ...
        if(count($user)>0){
                    if(!empty($reject_list)){
                             foreach($reject_list as $value) {
                                  ...
                                  /*  $reason_list = Rejectreason::where('id',$value->rejectreason_id)->get();
                                    $reason_name='';
                                    foreach ($reason_list as $r_name) {
                                        $reason_name = $r_name->reason;
                                    }*/
                                    $delivery_type_data = new_delivery_charges::where('id',$value->delivery_charges_id)->get();
                                    $delivery_type = '';
                                    foreach ($delivery_type_data as $dt) {
                    ...
                    } else if(!empty($reject_list_name)){
                            foreach($reject_list_name as $value) {
                                        $prescription_image = '';
                                        $image_list = Prescription::where('id',$value->prescription_id)->get();
                                        foreach ($image_list as $p_img) {
                                        if (!empty($p_img->image)) {
     ...
     public function cancel_order_list(Request $request){
        ...
        if (!empty($user_id) && !empty($search_text)) {
             ...
             if(count($name)>0){
              foreach ($name as $n) {
                   ...
                   $cancel_list_name = new_order_history::where(['process_user_id' => $user_id, 'order_status' => 'cancel','customer_id'=> $n->id])->orderBy('id', 'DESC')->get();
               }
           }else{
                $cancel_list = new_order_history::where('order_number', 'like', '%' .$search_text . '%')->where(['process_user_id' => $user_id, 'order_status' => 'cancel'])->orderBy('id', 'DESC')->get();
           }
        }else{
               $cancel_list =  new_order_history::where('process_user_id', $user_id)->where('order_status','cancel')->orderBy('id', 'DESC')->get();
        }
        ...
        if(count($user)>0){
                if(!empty($cancel_list)){
                         foreach($cancel_list as $value) {
                               ...
                               $name='';
                                ...
                                /*$cancel_list_reason = Cancelreason::where('id',$value->cancelreason_id)->get();
                                $cancel_name='';
                                foreach ($cancel_list_reason as $c_name) {
                                    $cancel_name = $c_name->reason;
                                }*/
                                $delivery_type_data = new_delivery_charges::where('id',$value->delivery_charges_id)->get();
                                    $delivery_type = '';
                                    foreach ($delivery_type_data as $dt) {
                ...
                } else if(!empty($cancel_list_name)){
                        foreach($cancel_list_name as $value) {
                                    $prescription_image = '';
                                    $image_list = Prescription::where('id',$value->prescription_id)->get();
                                    foreach ($image_list as $p_img) {
                                    if (!empty($p_img->image)) {
    ...
    public function complete_order_list(Request $request){
        ...
        if (!empty($user_id) && !empty($search_text)) {
             $name = new_order_history::where('name', 'like', '%' .$search_text . '%')->get();
             if(count($name)>0){
              foreach ($name as $n) {

                   $complete_list_name = new_order_history::where(['process_user_id' => $user_id, 'order_status' => 'complete','customer_id'=> $n->id])->orderBy('deliver_datetime', 'DESC')->get();
               }
           }else{
                $complete_list = new_order_history::where('order_number', 'like', '%' .$search_text . '%')->where(['process_user_id' => $user_id, 'order_status' => 'complete'])->orderBy('deliver_datetime', 'DESC')->get();
           }
        }else if(!empty($user_id) && !empty($start_date) && !empty($end_date) && !empty($delivery_boy)) { 
             $complete_list= new_order_history::whereBetween('created_at', [$start_date.' 00:00:00',$end_date.' 23:59:59'])->where(['process_user_id' => $user_id, 'order_status' => 'complete','deliveryboy_id'=> $delivery_boy])->orderBy('deliver_datetime', 'DESC')->get();

        }else if(!empty($user_id) && !empty($start_date) && !empty($end_date)) { 
            $complete_list= new_order_history::whereBetween('created_at', [$start_date.' 00:00:00',$end_date.' 23:59:59'])->where(['process_user_id' => $user_id, 'order_status' => 'complete'])->orderBy('deliver_datetime', 'DESC')->get();
        }else if(!empty($user_id) && !empty($delivery_boy)){
            $complete_list = new_order_history::where('deliveryboy_id', 'like', '%' .$search_text . '%')->where(['process_user_id' => $user_id, 'order_status' => 'complete','deliveryboy_id' => $delivery_boy])->orderBy('deliver_datetime', 'DESC')->get();
        }else if(!empty($user_id) && !empty($logistic_id)){
            $complete_list = new_order_history::where('logistic_user_id', 'like', '%' .$search_text . '%')->where(['process_user_id' => $user_id, 'order_status' => 'complete','logistic_user_id' => $logistic_id])->orderBy('deliver_datetime', 'DESC')->get();
        }else{
               $complete_list =  new_order_history::where('process_user_id', $user_id)->where('order_status','complete')->orderBy('deliver_datetime', 'DESC')->get();
        }
        ...
        if(count($user)>0){
                if(!empty($complete_list)){
                         foreach($complete_list as $value) {
                                    $prescription_image = '';
                                    $image_list = Prescription::where('id',$value->prescription_id)->get();
                                    foreach ($image_list as $p_img) {
                                    if (!empty($p_img->image)) {
                               ...
                               $mobile='';
                                ...
                                foreach ($deliveryboy_name as $d_name) {
                                    ...
                                    $deliveryboy_mobile = $d_name->mobile_number;
                                }
                                $delivery_type_data = new_delivery_charges::where('id',$value->delivery_charges_id)->get();
                                $delivery_type = '';
                                foreach ($delivery_type_data as $dt) {
                               ...
                               /* $update_date = new_orders::find($value->id);
                                $update_date->deliver_datetime=date('Y-m-d H:i:s');
                                $update_date->save();*/
                                $complete[] = [
                                    'order_id' => $value->order_id,
                                    'order_number' => $value->order_number,
                ...
                } else if(!empty($complete_list_name)){
                        foreach($complete_list_name as $value) {
                                    $prescription_image = '';
                                    $image_list = Prescription::where('id',$value->prescription_id)->get();
                                    foreach ($image_list as $p_img) {
                                    if (!empty($p_img->image)) {
        ...
        if (!empty($user_id) && !empty($search_text)) {
             ...
             if(count($name)>0){
              foreach ($name as $n) {
                   ...
                   $return_list_name = new_orders::where(['process_user_id' => $user_id, 'order_status' => 'incomplete','customer_id'=> $n->id])->orderBy('id', 'DESC')->get();
               }
           }else{
                $return_list = new_orders::where('order_number', 'like', '%' .$search_text . '%')->where(['process_user_id' => $user_id, 'order_status' => 'incomplete'])->orderBy('id', 'DESC')->get();
           }
        }else{
               $return_list =  new_orders::where('process_user_id', $user_id)->where('order_status','incomplete')->orderBy('id', 'DESC')->get();
        }
        $token =  $request->bearerToken();
        $user = new_pharma_logistic_employee::where(['id'=>$user_id,'api_token'=>$token])->get();
        if(count($user)>0){
                    if(!empty($return_list)){
                             foreach($return_list as $value) {
                                   ...
                                   $name='';
                                    ...
                                    /*$reason_list = Incompletereason::where('id',$value->incompletereason_id)->get();
                                    $reason_name='';
                                    foreach ($reason_list as $r_name) {
                                        $reason_name = $r_name->reason;
                                    }*/
                    ...
                    } else if(!empty($return_list_name)){
                            foreach($return_list_name as $value) {
                                        $prescription_image = '';
                                        $image_list = Prescription::where('id',$value->prescription_id)->get();
                                        foreach ($image_list as $p_img) {
                                        if (!empty($p_img->image)) {
    ...
    public function notification_seller(Request $request)
    {
        ...
        $notification_data = notification_seller::where('user_id',$user_id)->orderBy('id','DESC')->get();
