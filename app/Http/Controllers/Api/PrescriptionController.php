...
class PrescriptionController extends Controller
{
	public function prescription_list(Request $request)
	{
		...
		if(count($user)>0){
		if (!empty($searchtext)) {
			$prescription = Prescription::select('id', 'name', 'image', 'created_at')->where('name', 'like', '%'.$searchtext.'%')->where(['user_id'=>$user_id,"is_delete"=>"0"])->orderBy('id', 'DESC')->get();
		} else {
			$prescription = Prescription::select('id', 'name', 'image', 'created_at')->where(['user_id'=>$user_id,"is_delete"=>"0"])->orderBy('id', 'DESC')->get();
		}
		...
		$response['data'] = $prescription_arr;
		}else{
	    		$response['status'] = 401;
