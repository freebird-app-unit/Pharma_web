...
class UserPharmacyController_2 extends Controller
{
	...
	public function pharmacylist_newversion_2(Request $request)
	{
		$response = array();
	...
	public function GetPharmacyLogisticWise($search_array,$encryption){
		...
		if ($filter_ratings!='' && $filter_distance!=''){
			$isLogistics = 'true';
			...
			if($filter_discount == "5"){
				$isLogistics = 'true';
				$pharmacyFree = new_pharmacies::select('new_pharmacies.*',DB::raw("'false' as is_paid"), DB::raw('average_star AS rating'), DB::raw("6371 * acos(cos(radians(" . $current_latitude . ")) * cos(radians(new_pharmacies.lat)) * cos(radians(new_pharmacies.lon) - radians(" . $current_longitude . ")) + sin(radians(" . $current_latitude . ")) * sin(radians(new_pharmacies.lat))) AS distance"))->groupBy('new_pharmacies.id')->havingRaw('distance <= radius')->whereIn('new_pharmacies.id', $pharmacyIds)->where('new_pharmacies.is_active', 1)->where('new_pharmacies.average_star','=',$filter_ratings)->where('new_pharmacies.discount','<=',(int)$filter_discount);
				$queries = DB::getQueryLog();
				DB::connection()->enableQueryLog();
				$pharmacyPaid = new_pharmacies::select('new_pharmacies.*', DB::raw("'true' as is_paid"), DB::raw('average_star AS rating'), DB::raw("6371 * acos(cos(radians(" . $current_latitude . ")) * cos(radians(new_pharmacies.lat)) * cos(radians(new_pharmacies.lon) - radians(" . $current_longitude . ")) + sin(radians(" . $current_latitude . ")) * sin(radians(new_pharmacies.lat))) AS distance"))->groupBy('new_pharmacies.id')->havingRaw('distance >= radius')->whereIn('new_pharmacies.id', $pharmacyIds)->where('new_pharmacies.is_active', 1)->where('new_pharmacies.average_star','=',$filter_ratings)->where('new_pharmacies.discount','<=',(int)$filter_discount);
				$queries = DB::getQueryLog();
			...
			if($filter_discount == "5"){
				$isLogistics = 'true';
				$pharmacyFree = new_pharmacies::select('new_pharmacies.*',DB::raw("'false' as is_paid"), DB::raw('average_star AS rating'), DB::raw("6371 * acos(cos(radians(" . $current_latitude . ")) * cos(radians(new_pharmacies.lat)) * cos(radians(new_pharmacies.lon) - radians(" . $current_longitude . ")) + sin(radians(" . $current_latitude . ")) * sin(radians(new_pharmacies.lat))) AS distance"))->groupBy('new_pharmacies.id')->havingRaw('distance <= radius')->whereIn('new_pharmacies.id', $pharmacyIds)->where('new_pharmacies.is_active', 1)->where('new_pharmacies.discount','<=',(int)$filter_discount)->havingRaw('distance <='.$filter_distance.'');
				$queries = DB::getQueryLog();
				DB::connection()->enableQueryLog();
				$pharmacyPaid = new_pharmacies::select('new_pharmacies.*', DB::raw("'true' as is_paid"), DB::raw('average_star AS rating'), DB::raw("6371 * acos(cos(radians(" . $current_latitude . ")) * cos(radians(new_pharmacies.lat)) * cos(radians(new_pharmacies.lon) - radians(" . $current_longitude . ")) + sin(radians(" . $current_latitude . ")) * sin(radians(new_pharmacies.lat))) AS distance"))->groupBy('new_pharmacies.id')->havingRaw('distance >= radius')->whereIn('new_pharmacies.id', $pharmacyIds)->where('new_pharmacies.is_active', 1)->where('new_pharmacies.discount','<=',(int)$filter_discount)->havingRaw('distance <='.$filter_distance.'');
				$queries = DB::getQueryLog();
			...
			if($filter_discount == "5"){
				$isLogistics = 'true';
				$pharmacyFree = new_pharmacies::select('new_pharmacies.*',DB::raw("'false' as is_paid"), DB::raw('average_star AS rating'), DB::raw("6371 * acos(cos(radians(" . $current_latitude . ")) * cos(radians(new_pharmacies.lat)) * cos(radians(new_pharmacies.lon) - radians(" . $current_longitude . ")) + sin(radians(" . $current_latitude . ")) * sin(radians(new_pharmacies.lat))) AS distance"))->groupBy('new_pharmacies.id')->havingRaw('distance <= radius')->whereIn('new_pharmacies.id', $pharmacyIds)->where('new_pharmacies.is_active', 1)->where('new_pharmacies.average_star','=',$filter_ratings)->where('new_pharmacies.discount','<=',(int)$filter_discount)->havingRaw('distance <='.$filter_distance.'');
				$queries = DB::getQueryLog();
				DB::connection()->enableQueryLog();
				$pharmacyPaid = new_pharmacies::select('new_pharmacies.*', DB::raw("'true' as is_paid"), DB::raw('average_star AS rating'), DB::raw("6371 * acos(cos(radians(" . $current_latitude . ")) * cos(radians(new_pharmacies.lat)) * cos(radians(new_pharmacies.lon) - radians(" . $current_longitude . ")) + sin(radians(" . $current_latitude . ")) * sin(radians(new_pharmacies.lat))) AS distance"))->groupBy('new_pharmacies.id')->havingRaw('distance >= radius')->whereIn('new_pharmacies.id', $pharmacyIds)->where('new_pharmacies.is_active', 1)->where('new_pharmacies.average_star','=',$filter_ratings)->where('new_pharmacies.discount','<=',(int)$filter_discount)->havingRaw('distance <='.$filter_distance.'');
				$queries = DB::getQueryLog();
		...
		}else if($filter_ratings!=''){
			$pharmacyFree = $pharmacyFree->where(function ($query) use($filter_ratings) {
				$query->where('new_pharmacies.average_star','=',$filter_ratings);
			});
			if($isLogistics == 'true'){
			...
			if($filter_discount == "5"){
				...
				$query->where('new_pharmacies.discount','<=',(int)$filter_discount);
				});
				if($isLogistics == 'true'){
		...
		$final_array_merge = array_merge($new_arrival_sorting,$referral_code,$free_open, $free_close, $paid_open, $paid_close);
		...
		if($sortings == 'ratings'){
			usort($final_arr, "sort_pharmacy_array_ratings");
		}
