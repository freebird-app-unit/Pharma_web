...
class OutfordeliveryController extends Controller
{
    ...
    public function index()
    {
		...
		$data = array();
		$data['page_title'] = 'Assigned Orders';
		$data['page_condition'] = 'page_outfordelivery_logistic';
		$data['site_title'] = 'Assigned Orders | ' . $this->data['site_title'];
        return view('logistic.outfordelivery.index', $data);
    }
	public function getlist()
    {
		...
		->leftJoin('order_assign', 'orders.id', '=', 'order_assign.order_id')
		->where(['orders.order_status' => 'assign', 'order_assign.order_status' => 'assign']);
		if($user_type=='pharmacy'){
		...
		if(count($order_detail)>0){
			foreach($order_detail as $order){
				...
				$html.='<tr>
					<td><a href="'.url('/logistic/outfordelivery/order_details/'.$order->id).'"><img src="'.$image_url.'" width="50"/><span>'.$order->order_number.'</span></a></td>
			...
			$pagination.='<li class="page-item '.$prev.'">
						<a class="page-link" onclick="getoutfordeliverylistlogistic('.($page-1).')" href="javascript:;" tabindex="-1"> <i class="fa fa-angle-left"></i></a>
					</li>
					<li class="page-item '.$next.'">
						<a class="page-link" onclick="getoutfordeliverylistlogistic('.($page+1).')" href="javascript:;"><i class="fa fa-angle-right"></i></a>
					</li>';
	...
	public function order_details($id)
    {
		...
		$order_detail = DB::table('orders')->select('orders.*','delivery_charges.delivery_type as delivery_type','delivery_charges.delivery_price as deliveryprice', 'address_new.address as address','ua.address as pharmacyaddress','users.name as deliveryboyname')
		->leftJoin('users', 'users.id', '=', 'orders.deliveryboy_id')
		->leftJoin('users as ua', 'ua.id', '=', 'orders.pharmacy_id')
		->leftJoin('delivery_charges', 'delivery_charges.id', '=', 'orders.delivery_charges_id')
		->leftJoin('address_new', 'address_new.id', '=', 'orders.address_id')
        ...
        return view('logistic.outfordelivery.order_details', $data);
