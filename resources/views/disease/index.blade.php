...
@section('script')
	<script>
		...
		function loadForm(disease_id) {
			console.log(disease_id)
			$.ajax({
				headers: {'X-CSRF-Token': $('meta[name="csrf-token"]').attr('content')},
				url : base_url+ '/disease/load_form/'+disease_id,
				type: 'POST',
				success: function (data) { 
					if (!disease_id) {
						var title_msg = "Add Disease";
					} else {
						var title_msg = "Edit Disease";
					}
		...
		$(document.body).on('click','.save_btn',function(e) {
			...
			var formData =  new FormData($(".disease_form")[0]);
			
			
			if ($(".disease_form").valid()) { 
			
				ajax_request = $.ajax({
					headers: {'X-CSRF-Token': $('meta[name="csrf-token"]').attr('content')},
					url: base_url + "/disease_add",
					type: "POST",
					dataType : 'JSON',
					data:  formData,
					cache: false,
		...
		$(document).on('click', '.deleteDisease', function () {
			var obj = $(this);
			...
			swal({
				...
				$.ajax({
					headers: {'X-CSRF-Token': $('meta[name="csrf-token"]').attr('content')},
					url: base_url +'/disease/delete/' + CategoryId,
					type: "POST",
					success: function (res) {
						...
						if (res.status_code == '400') {
							...
							swal('Deleted!', 'Disease has been deleted.', "success");
