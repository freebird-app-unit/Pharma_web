...
<div class="row">
	<div class="col-sm-12">
		<div class="card-box">
			<form class="form-horizontal" method="POST" action="@if(isset($user_detail)){{ route('deliveryboy.edit',array('id'=>$user_detail->id)) }} @else{{ route('deliveryboy.create') }}@endif" id="user_detail-form" enctype="multipart/form-data">
				...
				<div class="form-group">
					<label class="control-label col-md-2 col-sm-2 col-xs-12" for="pharma_logistic_id">Pharmacy</label>
					<div class="col-md-8 col-sm-8 col-xs-12  @if($errors->has('pharma_logistic_id')) bad @endif">
						<select class="form-control" name="pharma_logistic_id" id="pharma_logistic_id">
							<option value="">Select Pharmacy</option>
							<?php
							foreach($pharmacy_list as $pharmacy){
									if(isset($user_detail) && (int)$user_detail->pharma_logistic_id == (int)$pharmacy->id){
										$sel = 'selected';
						...
						@if ($errors->has('pharma_logistic_id')) <div class="errors_msg">{{ $errors->first('pharma_logistic_id') }}</div>@endif
					</div>
				</div>
				<?php 
				}else{
				...
				<div class="form-group">
					...
					<div class="col-md-8 col-sm-8 col-xs-12  @if($errors->has('mobile_number')) bad @endif">
						<input type="text" placeholder="" class="form-control only_number" value="{{{ old('mobile_number', isset($user_detail) ? $user_detail->mobile_number : null) }}}" name="mobile_number" id="mobile_number">
				...
				<div class="form-group">
					...
					<div class="m-t-15 image_div col-md-2 col-sm-2 col-xs-12">
						<a href="javascript:void(0)">
							@if (file_exists(storage_path('app/public/uploads/new_delivery_boy/'.$user_detail->profile_image)))
								@php $image_path = asset('storage/app/public/uploads/new_delivery_boy/' . $user_detail->profile_image) @endphp
							@else 
								{{ $image_path = '' }}
							@endif
...
@section('script')
	...
	$(".deleteImage").click(function(){
            $.ajax({
                ...
                url:'{{ url("/deliveryboy/delete_image") }}',
                type: 'POST',
