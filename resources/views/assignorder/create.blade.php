...
<div class="row">
	<div class="col-sm-12">
		...
		@endif
			<form class="form-horizontal" method="POST" action="{{ route('assignorder.create') }}" id="user_detail-form" enctype="multipart/form-data">
			<input type="hidden" name="_token" value="{{ csrf_token() }}">
				...
				<div class="form-group">
					<label class="control-label col-md-2 col-sm-2 col-xs-12" for="assign">Assign Status<span class="required">*</span></label>
 ...
 $('#pharmacy_id').change(function(){
  ...
  if(PharmacyID){
    $.ajax({
      ...
      type:"GET",
      url:"{{url('assign/get-order-list')}}?pharmacy_id="+PharmacyID,
 ...
 $('#pharmacy_id').change(function(){
  ...
  if(PharmacyID){
    $.ajax({
      headers: {'X-CSRF-Token': $('meta[name="csrf-token"]').attr('content')},
      type:"GET",
      url:"{{url('assign/get-deliveryboy-list')}}?pharmacy_id="+PharmacyID,
      success:function(res){        
      if(res){
